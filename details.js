        ...
        carImageInclined.src = 'details-car-30.png';
        ...
        function update() {
            ...
            if (playerX + playerSpeedX >= platformX && playerX + playerSpeedX <= wallStartX) {
                // Player can move on the platform
                playerX += playerSpeedX;
            } else if (playerX + playerSpeedX >= wallStartX && playerX + playerSpeedX <= wallEndX) {
                playerSpeedX *= friction;
                playerX += playerSpeedX;
                playerY -= (playerSpeedX / (wallEndX - wallStartX)) * 270;
        
                // Check if the car is touching the inclined surface
                if (!isJumping && currentCarImage !== carImageInclined) {

                    isJumping = true;
                    currentCarImage = carImageInclined; // Change to inclined car image
                    playerWidth = inclinedCarWidth; // Change width
                    playerHeight = inclinedCarHeight; // Change height
                }
            } else {
                // Reset the car image and dimensions back to the regular one when not on the inclined surface
